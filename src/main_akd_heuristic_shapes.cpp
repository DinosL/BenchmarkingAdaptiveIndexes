#include "basics.h"
// #include "AIR.h"
#include "AKD_heuristic_shapes.h"
// #include "RTile.h"
...
//    auto seed = 1697617670;
    ...
    for(int i = 0; i < query_size; i++){
        ...
        found_count = akd.AdaptiveQuery(&(extended_queries)[i], &(original_queries[i]), rootCover);
        ...
        // }
            ...
            for(int ii =0; ii < DATA_COUNT; ii++){
                ...
                // if(Overlap(queries[i].m_min, queries[i].m_max, data[ii].m_values)){
                    // cout << m_data_arr_rects[ii].m_min[0] << " " << m_data_arr_rects[ii].m_min[1] << " " << m_data_arr_rects[ii].m_max[0] << " " << m_data_arr_rects[ii].m_max[1] << endl;
                    lin_count++;
                }
            }
            ...
            // if(abs(lin_count - found_count) > 1){
                cout << "ERROR!!! found:  " << found_count << " instead of " << lin_count << endl;
                // cout << "Query " << i << endl;

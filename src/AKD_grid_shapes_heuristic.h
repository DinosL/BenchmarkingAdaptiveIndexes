...
class AKDTree{
    public:
        ...
        int AdaptiveQuery(Rect *extendedQuery, Rect* originalQuery, int partitionsPerDimensio, int thisCellId, Rect cover);
        int AdaptiveQueryDuplicates(Rect *extendedQuery, Rect* originalQuery, float partitionExtent, int partitionsPerDimensio, int thisCellId, Rect cover);
...
int AKDTree::AdaptiveQuery(Rect *extendedQuery, Rect* originalQuery, int partitionsPerDimension, int thisCellId, Rect cover){
    int foundCount = 0;
...
int AKDTree::AdaptiveQueryDuplicates(Rect *extendedQuery, Rect* originalQuery, float partitionExtent, int partitionsPerDimension, int thisCellId, Rect cover){
    int foundCount = 0;
            ...
            for(int i =0; i < 2*NUMDIMS; i++)
            {
                ...
                if(createdLeaves.size() > 0)
                {
                    ...
                    if(nodeToBeCracked->dataArrayRightPointer - nodeToBeCracked->dataArrayLeftPointer > CRACKTHRESHOLD)
                    {
                        ...
                        SplitNodeInPlace(nodeToBeCracked, median, chosenStochasticAxis);
                        // if(flag) 
                        // {
                        //     actuallyCracked++;

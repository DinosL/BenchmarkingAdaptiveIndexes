...
class AKDTree{
    public:
        ...
        int AdaptiveQuery(Rect* extendedQuery, Rect* originalQuery, Rect cover);
    ...
    private:
        ...
        void AdaptiveQuery(KDNode* a_node, Rect *query, int &foundCount, vector<Rect> &candidates, Rect cover);
...
int AKDTree::partition2D(int start, int end, int axis, ELEMTYPE pivot){
    ...
    while(i <= j && j > 0){
        ...
        if(m_data_arr_rects[i].m_min[axis] < pivot){
            i++;
        }
        else{
            // cout << "Is " << m_data_arr[i][axis] << " > " << pivot << endl;
            while(j > 0 && j >= i && m_data_arr_rects[j].m_min[axis] >= pivot){      // TODO can remove j > 0, i think
                j--;
            }
            // cout << "I,J " << i << " " << j << endl; 
            if(i < j){
                ...
                #endif
                // cout << "SWAP!!!!\n";
                std::swap(m_data_arr_rects[i], m_data_arr_rects[j]);
...
int AKDTree::ChooseAxisLargestSideMid(Rect node_cover, Rect query_cover) {
    ...
    // cout << "Query is " << query_cover.m_min[0] << " " << query_cover.m_min[1] << endl;
...
int AKDTree::AdaptiveQuery(Rect *extendedQuery, Rect *originalQuery, Rect cover){
    int foundCount = 0;
    ...
    AdaptiveQuery(&m_root, extendedQuery, foundCount, candidates, cover);
    int realResults = 0;
    ...
    // cout << " Size = " << candidates.size() << " Found count: " << foundCount << " real: " << realResults << endl;
    // cout << foundCount << "\t" << realResults << endl;

    return realResults;
}


void AKDTree::AdaptiveQuery(KDNode* a_node, Rect *query, int &foundCount, vector<Rect> &candidates, Rect cover){
    if(a_node->leafness){
        // is a leaf
        // should crack on the bounds
        // also add to the foundcount
        sizeOfPieceScan += a_node->dataArrayRightPointer - a_node->dataArrayLeftPointer;
        // cout << a_node->dataArrayRightPointer - a_node->dataArrayLeftPointer << endl;
        ...
        if(a_node->dataArrayRightPointer - a_node->dataArrayLeftPointer > CRACKTHRESHOLD){
            ...
            for(int i =0; i < 2*NUMDIMS; i++){
                ...
                if(i == ((2 * NUMDIMS) - 1)){
                    ...
                    choice = ChooseAxisLargestSideMid(cover, *query);
                }
                if(choice < 0) break;
                ...
                if(mm == 0)
                {
                    SplitNodeInPlace(thisNode, query->m_min[chosen_axis], chosen_axis);
                    if(!thisNode->leafness)
                    {
                        #ifdef STOCHASTIC
                            // if(i == 0)
                            ...
                            //         // float X = m_data_arr_rects[thisNode->dataArrayLeftPointer + ( rand() % (thisNode->dataArrayRightPointer - thisNode->dataArrayLeftPointer + 1 ))].m_min[chosenStochasticAxis];
                            //         // float X2 = m_data_arr_rects[thisNode->dataArrayLeftPointer + ( rand() % (thisNode->dataArrayRightPointer - thisNode->dataArrayLeftPointer + 1 ))].m_min[chosenStochasticAxis];
                            //         // float X3 = m_data_arr_rects[thisNode->dataArrayLeftPointer + ( rand() % (thisNode->dataArrayRightPointer - thisNode->dataArrayLeftPointer + 1 ))].m_min[chosenStochasticAxis];
                            ...
                            //         float median = max(min(m_data_arr_rects[rndValues[0]].m_min[chosenStochasticAxis],m_data_arr_rects[rndValues[1]].m_min[chosenStochasticAxis]), min(max(m_data_arr_rects[rndValues[0]].m_min[chosenStochasticAxis],m_data_arr_rects[rndValues[1]].m_min[chosenStochasticAxis]),m_data_arr_rects[rndValues[2]].m_min[chosenStochasticAxis]));
                            //         // cout <<"Median is " << X << endl;
                            //         bool flag = SplitNodeInPlace(thisNode->leftChild, median, chosenStochasticAxis);
                            //         // if(flag) actuallyCracked++;
                            //     }
                            // }
                ...
                {
                    SplitNodeInPlace(thisNode, query->m_max[chosen_axis], chosen_axis);
                    if(!thisNode->leafness)
                    {
                        #ifdef STOCHASTIC
                            // if(i == 0)
            ...
            for(int ii = thisNode->dataArrayLeftPointer; ii < thisNode->dataArrayRightPointer; ++ii){
                if(Overlap(query, &(m_data_arr_rects)[ii])){
                    candidates.push_back(m_data_arr_rects[ii]);
                    foundCount++;
                }
            }
            #ifdef TIMEBREAKDOWN
            ...
            // cout << " right, left, thresh " << thisNode->dataArrayRightPointer << " " << thisNode->dataArrayLeftPointer << " " << CRACKTHRESHOLD << endl;
        ...
        if(query->m_min[a_node->axis] < a_node->pivotValue){
            AdaptiveQuery(a_node->leftChild, query, foundCount, candidates, leftCover);
        }
        if(query->m_max[a_node->axis] >= a_node->pivotValue){
            AdaptiveQuery(a_node->rightChild, query, foundCount, candidates, rightCover);
        }
    }
}

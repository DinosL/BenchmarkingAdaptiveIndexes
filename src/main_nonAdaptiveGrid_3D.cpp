...
int getCellId(ELEMTYPE x, ELEMTYPE y, ELEMTYPE z, int numCellsPerDimension) 
{
    ...
    // return (z + y * numCellsPerDimension + x * numCellsPerDimension * numCellsPerDimension);
...
int main(int argc, char **argv){
    ...
    int totalPartitions = partitionsPerDimension*partitionsPerDimension*partitionsPerDimension;
    int xStartCell, yStartCell, xEndCell, yEndCell, zStartCell, zEndCell, startCellId, endCellId, thisCellId, thisQueryResult;
    int linCount = 0;
    ...
    for(int i = 0; i < query_size; i++)
    {
        ...
        // cout << "x " << xStartCell << " " << xEndCell << endl;
        // cout << "y " << yStartCell << " " << yEndCell << endl;
        // cout << "z " << zStartCell << " " << zEndCell << endl;
            ...
            for(int yIndex = yStartCell; yIndex <= yEndCell; yIndex++)
            {
                for(int zIndex = zStartCell; zIndex <= zEndCell; zIndex++)
                {
                    ...
                    if(xIndex == xStartCell || xIndex == xEndCell || yIndex == yStartCell || yIndex == yEndCell || zIndex == zStartCell || zIndex == zEndCell) 
                    {
                        ...
                        {
                            if(cellData[thisCellId][ii*NUMDIMS] < queries[i].m_min[0] || cellData[thisCellId][ii*NUMDIMS] > queries[i].m_max[0] || cellData[thisCellId][ii*NUMDIMS+1] < queries[i].m_min[1] || cellData[thisCellId][ii*NUMDIMS+1] > queries[i].m_max[1] || cellData[thisCellId][ii*NUMDIMS+2] < queries[i].m_min[2] || cellData[thisCellId][ii*NUMDIMS+2] > queries[i].m_max[2])
                            {
                                // cout << "1 " << cellData[thisCellId][ii*NUMDIMS] << " " << queries[i].m_min[0] << " " << queries[i].m_max[0] << endl;
                                // cout << "2 " << cellData[thisCellId][ii*NUMDIMS+1] << " " << queries[i].m_min[1] << " " << queries[i].m_max[1] << endl;
                                // cout << "3 " << cellData[thisCellId][ii*NUMDIMS+2] << " " << queries[i].m_min[2] << " " << queries[i].m_max[2] << endl;
                                continue;
                            }
                            // cout << "1 " << cellData[thisCellId][ii*NUMDIMS] << " " << queries[i].m_min[0] << " " << queries[i].m_max[0] << endl;
                            // cout << "2 " << cellData[thisCellId][ii*NUMDIMS+1] << " " << queries[i].m_min[1] << " " << queries[i].m_max[1] << endl;

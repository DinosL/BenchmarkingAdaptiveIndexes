...
class ProgressiveIndex: public AbstractIndex {
  ...
  // void initialize_max_table(Table *table_to_copy) override;

  void adapt_index(Query &query) override;

  unique_ptr<Table> range_query(Query &query, Query &original_query) override;

  unique_ptr<Table> progressive_quicksort(Query &query, Query &original_query);
...
private:
    ...
    unique_ptr<Table> progressive_quicksort_create(Query &query, ssize_t &remaining_swaps, Query &original_query);

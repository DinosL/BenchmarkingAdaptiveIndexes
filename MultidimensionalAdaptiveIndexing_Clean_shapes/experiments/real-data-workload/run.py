...
# print(len(sys.argv))
if len(sys.argv) != 10:
    print("Usage: python3 run.py [datafile] [queryfile] [#queries] [#repetitions] [#datasize] [#dimensions] [output_folder_path] [partition_size] [extended_workload_file]")
    sys.exit(1)

CURRENT_DIR = os.getcwd()

DATA_FILE = f"{CURRENT_DIR}"+"/"+sys.argv[1]
ORIGINAL_WORKLOAD_FILE = f"{CURRENT_DIR}"+"/"+sys.argv[2]
...
EXTENDED_WORKLOAD_FILE = f"{CURRENT_DIR}"+"/"+sys.argv[9]
...
dinos_exp = {
        "name": "dinos",
        "data": DATA_FILE,
        "original_workload": ORIGINAL_WORKLOAD_FILE,
        "extended_workload": EXTENDED_WORKLOAD_FILE,
        "number_of_rows": ROWS,
...
#         "delta": "0.6",
#         "result": f"{OUTPUT_FOLDER}times_PKD_partition{PARTITION_SIZE}_",
#         "run_times" : sys.argv[4],
#     }
# )

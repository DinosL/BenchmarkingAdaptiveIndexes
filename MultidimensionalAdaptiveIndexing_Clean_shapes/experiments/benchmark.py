        ...
        with CD(self.bin_dir):
            for experiment in self.experiments:
                for run in self.runs:
                    for i in range(int(run['run_times'])):
                        command = [
                            "./main",
                            "-w", experiment['original_workload'],
                            "-b", experiment['extended_workload'],
                            "-d", experiment['data'],
                            "-i", run['algorithm_id'],
                            "-r", experiment['repetitions'],
                            # "-s", f"{run['result']}-{experiment['exp_id']}.csv",
                            "-s", f"{run['result']}"+str(i+1)+".txt",
                            "-p", run.get('partitions_size', "1024"),
                            "-a", run.get('delta', "0"),
                            "-x", experiment['dimensions'],
                            "-k", experiment['number_of_rows'],
                            "-l", experiment['number_of_queries'],
                            run.get('extra_flags', '')
                            ]

...
unique_ptr<Table> CrackingKDTreePreProcessing::range_query(Query &query, Query &original_query) {
    // ******************
    auto start = measurements->time();
    ...
    // FATEMEH
    FullScanCandidateList::refine_window_query_extension(table.get(), table_max.get(), original_query, partitions);
    // FATEMEH OUT

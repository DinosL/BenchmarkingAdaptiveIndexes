...
//! Workload agnostic is only used if we already refined dependent on workload
//! or we already answered the query
void ProgressiveIndex::workload_agnostic_refine(Query &query,
  ...
  while (node_being_refined < refinement_nodes->size() && remaining_swaps) {
    ...
    if (node->current_start >= node->current_end) {
      node->position = column[node->current_start] >= node->key
                           ? node->current_start
                           : node->current_start + 1;
      node_being_refined++;
      size_t next_dimension =
          node->column == num_dimensions - 1 ? 0 : node->column + 1;
      ...
      if (current_end >= current_start &&
          current_end - current_start >= minimum_partition_size) {
        float pivot =
            find_avg(table.get(), next_dimension, current_start, current_end);
        node->setLeft(make_unique<KDNode>(next_dimension, pivot, current_start,
                                          current_end));
      ...
      if (current_end >= current_start &&
        ...
        float pivot =
            find_avg(table.get(), next_dimension, current_start, current_end);
        node->setRight(make_unique<KDNode>(next_dimension, pivot, current_start,
                                           current_end));
        refinement_nodes->push_back(node->right_child.get());
      }
    }
  }
  //    cout << node_being_refined << " of : " << refinement_nodes->size() <<
  //    endl;
...
void ProgressiveIndex::workload_dependent_refine(Query &query,
                                                 ssize_t &remaining_swaps) {
  start_time = measurements->time();
  ...
  //! This means we fully refined all nodes related to the query, now we can use
  //! the remaining budget to refine random nodes
...
unique_ptr<Table>
ProgressiveIndex::progressive_quicksort_create(Query &query, ssize_t &remaining_swaps, Query &original_query) {
  ...
  //! We start by getting a candidate list to the upper part of our indexed
  //! table
  start_time = measurements->time();
  ...
  auto indexMaxColumns = table->max_columns[dim]->data;
  auto originalColumn = originalTable->columns[dim]->data;
  auto originalMaxColumn = originalTable->max_columns[dim]->data;
  ...
  if (low <= root->key) {
    ...
    for (dim = 1; dim < query.predicate_count(); ++dim) {
      ...
      indexMaxColumns = table->max_columns[dim]->data;
  ...
  if (high >= root->key) {
    ...
    for (dim = 1; dim < query.predicate_count(); ++dim) {
      ...
      for (size_t i = 0; i < down.size; i++) {
        int matching =
            indexColumn[down.get(i)] >= low && indexColumn[down.get(i)] <= high;
  ...
  if (cur_interactivity_threshold > 0 &&
      (tree->root->start != tree->root->current_start ||
  ...
  //! It has elements from when we start swapping in this partition till the end
  //! of the table
  start_time = measurements->time();
  ...
  for (size_t i = current_position; i < next_index; i++) {
    ...
    indexMaxColumns[root->current_start] = originalMaxColumn[i];
    indexMaxColumns[root->current_end] = originalMaxColumn[i];
  ...
  for (dim = 1; dim < query.predicate_count(); ++dim) {
    ...
    originalMaxColumn = originalTable->max_columns[dim]->data;
    size_t initial_low_cur = initial_low;
    size_t initial_high_cur = initial_high;
    //! First we copy the elements of the other columns, until where we stopped
    //! skipping
    bit_idx = 0;
    auto mid_idx = 0;
    ...
    for (size_t i = current_position; i < next_index; i++) {
      ...
      indexMaxColumns[initial_low_cur] = originalMaxColumn[i];
      indexMaxColumns[initial_high_cur] = originalMaxColumn[i];
  ...
  if (next_index == table_size) {
    ...
    indexMaxColumns = table->max_columns[0]->data;
    root->position = indexColumn[root->current_start] >= root->key
                         ? root->current_start
                         : root->current_start + 1;
    ...
    for (dim = 1; dim < query.predicate_count(); ++dim) {
      ...
      for (size_t i = 0; i < original.size; i++) {
        int matching = originalColumn[original.get(i)] >= low &&
                       originalColumn[original.get(i)] <= high;
  ...
  if (cur_interactivity_threshold > 0 &&
    ...
    while (scan_time + adaptation_time + index_search_time <
      ...
      for (size_t i = current_position; i < next_index; i++) {
        ...
        indexMaxColumns[root->current_start] = originalMaxColumn[i];
        indexMaxColumns[root->current_end] = originalColumn[i];
      ...
      for (dim = 1; dim < query.predicate_count(); ++dim) {
        ...
        size_t initial_low_cur = initial_low;
        size_t initial_high_cur = initial_high;
        //! First we copy the elements of the other columns, until where we
        //! stopped skipping
        bit_idx = 0;
        for (size_t i = current_position; i < next_index; i++) {
          indexColumn[initial_low_cur] = originalColumn[i];
      ...
      if (next_index == table_size) {
        ...
        root->position = root->current_start;
        root->setLeft(
            make_unique<KDNode>(dim, pivot, current_start, current_end));
        //! Right node
        pivot = find_avg(table.get(), dim, current_start + 1, table_size - 1);
        current_start = root->current_start;
        current_end = table_size - 1;
...
unique_ptr<Table> ProgressiveIndex::progressive_quicksort(Query &query, Query &original_query) {
    log_q = false;
  ...
  if (tree->root->start == tree->root->current_start &&
    ...
    auto result = progressive_quicksort_create(query, remaining_swaps, original_query);
    interactivity_threshold = scan_time + index_search_time + adaptation_time;
    cur_interactivity_threshold = interactivity_threshold;
    measurements->append("scan_time", std::to_string(scan_time));
    measurements->append("index_search_time",
                         std::to_string(index_search_time));
    measurements->append("adaptation_time", std::to_string(adaptation_time));
    measurements->append("result", std::to_string((int)result->columns[1]->data[0]));
    return result;
    ...
    num_queries_over--;
    auto result = progressive_quicksort_create(query, fq_remaining_swaps, original_query);
    cur_interactivity_threshold =
        scan_time + index_search_time + adaptation_time;
    measurements->append("scan_time", std::to_string(scan_time));
    measurements->append("index_search_time",
                         std::to_string(index_search_time));
    measurements->append("adaptation_time", std::to_string(adaptation_time));
    ...
    interactivity_threshold_over =
        scan_time + index_search_time + adaptation_time;
    cur_interactivity_threshold = interactivity_threshold;
    if (scan_time > interactivity_threshold) {
      cur_interactivity_threshold = interactivity_threshold_over;
      interactive_threshold_is_over = true;
    }
    measurements->append("scan_time", std::to_string(scan_time));
    measurements->append("index_search_time",
                         std::to_string(index_search_time));
  ...
  measurements->append("scan_time", std::to_string(scan_time));
  measurements->append("index_search_time", std::to_string(index_search_time));
  measurements->append("adaptation_time", std::to_string(adaptation_time));
...
ProgressiveIndex::ProgressiveIndex(std::map<std::string, std::string> config) {
  ...
  if (config.find("interactivity_threshold_is_time") == config.end()) {
    interactive_threshold_is_time = false;
  } else {
...
double ProgressiveIndex::get_delta(Query &query) {
  ...
  double time = cur_interactivity_threshold -
                (scan_time + adaptation_time + index_search_time);
  double scan_speed = READ_ONE_PAGE_SEQ_MS * page_count * table->col_count() +
                      RANDOM_ACCESS_PAGE_MS * table->col_count();
  double pivot_speed = (READ_ONE_PAGE_SEQ_MS + WRITE_ONE_PAGE_SEQ_MS) *
                           page_count * table->col_count() +
                       RANDOM_ACCESS_PAGE_MS * table->col_count();
  //! Creation Phase
  if (tree->root->noChildren()) {
    ...
    double rho = (double)(table->row_count() - tree->root->current_start -
                          (tree->root->end - tree->root->current_end)) /
                 (double)table->row_count();
    for (size_t j = 0; j < ITERATIONS; j++) {
      estimated_time = ((rho - estimated_delta) * scan_speed +
                        estimated_delta * pivot_speed) /
                       1000.0;
    ...
    double refine_speed =
        WRITE_ONE_PAGE_SEQ_MS * page_count * table->col_count() +
        RANDOM_ACCESS_PAGE_MS * table->col_count();
    //! figure out alpha
    ...
    for (size_t j = 0; j < ITERATIONS; j++) {
      estimated_time =
          (lookup_speed + alpha * scan_speed + estimated_delta * refine_speed) /
          1000.0;
      if (estimated_time > time) {
        estimated_delta -= offset;
      } else {
...
double ProgressiveIndex::get_delta_react() {
  ...
  if (tree->root->noChildren()) {
    double pivot_speed =
        first_col_cr * page_count + extra_col_cr * table->col_count() - 1;
    double time = cur_interactivity_threshold -
                  (scan_time + adaptation_time + index_search_time);
    if (time < 0) {
      return 0;
...
void ProgressiveIndex::initialize(Table *table_to_copy) {
  //! Check partition size, we change it to guarantee it always partitions all
  ...
  //! We don't time this because could be assume this is collected during data
  //! loading
  ...
  // std::cout << "Row, col " << table_to_copy->row_count() << " " << table_to_copy->col_count() << std::endl;
  for (size_t i = 0; i < table->col_count(); i++) {
    table->columns[i]->size = table_to_copy->row_count();
    table->max_columns[i]->size = table_to_copy->row_count();
  }
  ...
  measurements->append("initialization_time",
    ...
    if (num_queries_over > 0) {
    //! find how big piece can be
    double page_count =
        (table->row_count() / ELEMENTS_PER_PAGE) +
        (table->row_count() % ((int)ELEMENTS_PER_PAGE) != 0 ? 1 : 0);
    size_t ITERATIONS = 20;
    double offset = page_count / 2;
    for (size_t j = 0; j < ITERATIONS; j++) {
      double scan_speed =
          READ_ONE_PAGE_SEQ_MS * page_count * table->col_count() +
          RANDOM_ACCESS_PAGE_MS * table->col_count();
      if (scan_speed > interactivity_threshold) {
        page_count -= offset;
      } else {
        page_count += offset;
      }
    ...
    //! find upper_bound of swaps per query

    int min_piece = page_count * ELEMENTS_PER_PAGE;
    int aux_size = table->row_count();
    while (min_piece < aux_size) {
      fq_remaining_swaps = table->row_count();
      aux_size /= 2;
    }
...
//! Here we don't do anything since adapt and scan are the same process in
//! progressive indexing
void ProgressiveIndex::adapt_index(Query &query) {}

unique_ptr<Table> ProgressiveIndex::range_query(Query &query, Query &original_query) {
  return progressive_quicksort(query, original_query);
}
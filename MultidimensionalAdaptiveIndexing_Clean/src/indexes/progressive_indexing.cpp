...
unique_ptr<Table>
ProgressiveIndex::progressive_quicksort_create(Query &query,
                                               ssize_t &remaining_swaps) {
  ...
  if (low <= root->key) {
    ...
    for (dim = 1; dim < query.predicate_count(); ++dim) {
      ...
      for (size_t i = 0; i < up.size; i++) {
        int matching =
            indexColumn[up.get(i)] >= low && indexColumn[up.get(i)] <= high;
...
void ProgressiveIndex::initialize(Table *table_to_copy) {
  ...
  while (minimum_partition_size >
         table_to_copy->row_count() / pow(2, table_to_copy->col_count())) {

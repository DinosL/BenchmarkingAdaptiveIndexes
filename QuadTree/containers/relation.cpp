...
void Relation::loadQueries(const char *filename)
{
    RecordId id = 0;
    ...
    while (getline( file, line ) )
    {
        ...
        {      
            ...
            // this->maxX = std::max(this->maxX, x1);
            // this->minY = std::min(this->minY, y1);
            // this->maxY = std::max(this->maxY, y1);

            // minXmbr = std::min(minXmbr, x1);
            // maxXmbr = std::max(maxXmbr, x1);
            // minYmbr = std::min(minYmbr, y1);
            // maxYmbr = std::max(maxYmbr, y1);
            ...
            // minXmbr = std::min(minXmbr, x2);
            // maxXmbr = std::max(maxXmbr, x2);
            // minYmbr = std::min(minYmbr, y2);
            // maxYmbr = std::max(maxYmbr, y2);
            // cout << "ADDING " << x1 << " " << y1 << " " << x2 << " " << y2 << endl;
            this->emplace_back(id, x1, y1, x2, y2);      
            id++;
        }
...
void Relation::normalize(Coord minX, Coord maxX, Coord minY, Coord maxY, Coord maxExtent) {
    ...
    { 
        ...
        // cerr << rec.xStart << " " << rec.yStart << " " << rec.xEnd << " " << rec.yEnd << endl;
    }
}

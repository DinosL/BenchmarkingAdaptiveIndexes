...
int main(int argc, char **argv)
{
    ...
    #pragma omp parallel sections
    {
        #pragma omp section
        {
            R.loadQueries(argv[optind]);
        }
        #pragma omp section
        {
            S.loadQueries(argv[optind+1]);
        }
    }
            ...
            if( level == -1 ){
                ...
                for (size_t i = 0; i < S.numRecords; i++){
                    for(int o = 0; o < NUM_ITERATIONS; o++) {
                        ...
                        // cout << "Query " << S[i].xStart << " " << S[i].yStart << " " << S[i].xEnd << " " << S[i].yEnd << endl;
            ...
            else
            {
                ...
                for (size_t i = 0; i < S.numRecords; i++){
                    for(int o = 0; o < NUM_ITERATIONS; o++) {
                        ...
                        cout << result <<"\t" << queryExecutionTime+timeCounting <<"\t"<<timeIndexingOrPartitioning<< endl;
